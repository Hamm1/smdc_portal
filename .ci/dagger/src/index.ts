...
export class Ci {
  ...
  linux(src: Directory): File {
    return dag
      ...
      .withExec(["./nsolid_setup_deb.sh","25"])
  ...
  tests(src: Directory): File {
    return dag
      ...
      .withMountedDirectory("/app", src)
      .withWorkdir("/app")
      .withExec(["cargo", "test", "--manifest-path", "src-tauri/Cargo.toml"])
  }

  @func()
  playwright(src: Directory): File {
    return dag
      .container({ platform: "linux/amd64" as Platform })
      .from("ubuntu:22.04")
      .withExec(["apt-get", "update"])
      .withExec(["apt-get", "install", "-y","unzip", "curl",])
      .withExec(["curl","-SLO","https://deb.nodesource.com/nsolid_setup_deb.sh"])
      .withExec(["rm","-f","/usr/share/keyrings/nodesource.gpg"])
      .withExec(["chmod","500","nsolid_setup_deb.sh"])
      ...
      .withExec(["bun", "run", "test.e2e", "--reporter=line"])
  }
}
